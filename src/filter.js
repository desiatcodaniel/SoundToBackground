...
export async function filterBG(imageToSegment, filter) {
  ...
  if (filter === "original")
    resultArea.style.background = `url(${backgrounds[0]})`;
  if (filter === "tropical")
    resultArea.style.background = `url(${backgrounds[1]})`;
  if (filter === "romantic")
    resultArea.style.background = `url(${backgrounds[2]})`;
